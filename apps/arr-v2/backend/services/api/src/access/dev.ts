...
const query = async (sql: string, values?: unknown[]) => {
  if (sql.includes('CREATE TABLE') && !values) { await db.exec(sql); return { rows: [] }; }
  return db.query(sql, values);
};
const adapter = { query, connect: async () => ({ query, release() {} }) } as unknown as Pool;
...
const server = createServer(async (req, res) => {
  ...
  if (!await handler(req, res, path)) { res.writeHead(404); res.end(); }
});
server.listen(Number(process.env.PORT || 19327), '127.0.0.1', () => console.log('T-schutes local: http://127.0.0.1:' + (process.env.PORT || 19327) + '/api/access-demo/'));
