...
## Review polish, clipboard and numbered batches
- Change password sits below the identity band; username is 10% larger. Enabled white buttons use blue text; primary green/white and disabled gray remain distinct. Implemented ? still completes immediately without a dialog.
- Inline Original/Current/Owner sections collapse duplicated Original/Current by default; Owner stays open. Word-level inert-text comparisons highlight additions in yellow and strike through yellow removals in the preceding version. Comments remain separate. Comparisons over one million token pairs use a bounded common-prefix/suffix comparison to avoid blocking the page.
- Priority changes animate for 1.1 seconds, skipping animation for reduced-motion users. Persistence occurs before animation; a failed save never visually changes the order.
- Paste image from clipboard supports compatible browser clipboard permissions. The focused dashed area and ticket text fields also accept image Ctrl/Cmd+V. Devices denying clipboard access retain keyboard paste/file selection. No image must be saved to disk first.
- New batches receive permanent numbers starting at 3 under the database transaction lock. Historic titles/briefs are not renumbered. A sequence record prevents number reuse.
- Calendar hours are rounded down: earliest final approval to all-member completion, and maximum latest-submitted-revision to final approval. The first clock updates in the UI and freezes when all batch members are completed. Explicit approval and submitter-revision timestamps determine the snapshot; undocumented historic approvals show Unavailable rather than invented values. Manual archive implementation dates do not drive these metrics. Batch requirement snapshots remain unchanged on completion.

## Batch 3: filtering, completion animation and historical timing
- All dates toggles to Restore Date Range when both bounds are empty. Restore uses local-calendar today minus fourteen days through today, not a previously edited custom range.
- Successful Implemented ? with Hide Completed enabled replaces the ticket and expanded detail with a white space that shrinks over one second. Persistence failure never removes the row; reduced-motion users skip animation. With the filter off, completed rows stay visible.
