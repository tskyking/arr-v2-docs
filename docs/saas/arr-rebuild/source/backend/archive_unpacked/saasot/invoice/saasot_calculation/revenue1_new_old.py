...
# ---------------------revenue according to over life of subscription---------------------------
    if obj.productp_service.revenue_type.revenue_type == "over life of subscription":
        ...
        while current_date <= new_end_date or (end_date.month == current_date.month and end_date.year == current_date.year):
            revenue = {}
...
# ---------------------deferred_revenue according to over life of subscription---------------------------
    if obj.productp_service.revenue_type.revenue_type == "over life of subscription":
        ...
        while current_date <= new_end_date or (end_date.month == current_date.month and end_date.year == current_date.year):
            ...
            if parser.parse(str(obj.tansaction.order_close_data)).date() > current_date and current_date.month != parser.parse(str(obj.tansaction.order_close_data)).date().month:
                negative_value -= rev[d]['value']
                revenue1['value'] = negative_value
                value -= rev[d]['value']
            else:
