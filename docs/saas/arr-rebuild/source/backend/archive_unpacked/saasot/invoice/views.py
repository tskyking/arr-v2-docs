...
from .models import Transaction, Item, Calculation, CloseDate, ArrGracePeriod, CalculationMonths
    ...
    def post(self, request):
        ...
        if data['csv_file']:
            ...
            if serializer.is_valid():
                undefiend_service = load_membership_data(data['csv_file'], request.user)
...
#                     data['csv_file'], request.user)
#                 # threading.Thread(target=load_membership_data, args=[data['csv_file'], request.user]).start()
#                 if undefiend_service.get('error', None):
#                     return Response({'message': str(undefiend_service['error'])}, status=status.HTTP_400_BAD_REQUEST)
#                 return Response({"undefiend_service": undefiend_service, "message": "Sucesfully upload"}, status=status.HTTP_200_OK)
...
# -----------------------creating item here----------------------------
            ...
            if item_serializer.is_valid():
                ...
                return Response({"message": "Sucesfully Created"}, status=status.HTTP_201_CREATED)
            else:
...
class ArrGracePeriodViewset(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        if serializer.is_valid():
            # Delete previous records for the company and save the new one
            ArrGracePeriod.objects.filter(company=request.user.company).delete()
            instance = serializer.save()

            # Trigger the calculation handler
    ...
    def update(self, request, *args, **kwargs):
        return Response("this nethod is not allowed")
...
class ItemRevenueView(generics.ListAPIView):
    """
    A viewset that calculate the revenue and other things on item based
    """
