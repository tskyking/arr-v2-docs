...
# ---------------------revenue according to over life of subscription---------------------------
    if obj.productp_service.revenue_type.revenue_type == "over life of subscription":
        ...
        # irretate start_date to end_date to assign value to each month
...
# ---------------------revenue according to over the expected life of the customer---------------------------
        ...
        # print('((((((((((((((((((((((((((((((( OBJ Date )))))))))))))))))))))))))))))))')
        # print(obj.created_at, ' > ', obj.productp_service.revenue_type.updated_at)

        # Assuming obj.created_at is already a datetime object
        created_at = obj.created_at

        # Convert obj.productp_service.revenue_type.updated_at (string) to a datetime object
        updated_at = parser.parse(obj.productp_service.revenue_type.updated_at)
        ...
        if created_at > updated_at:
            ...
            try:
                ...
                mth = 0
            # if obj.tansaction.customer_name == '#1 Green Township (IN) Volunteer Fire Department':
            #     print(1+"i")
            # mth = obj.productp_service.revenue_type.months  #yhan krna h month model call
            ...
            delta = end_date - start_date
            days = delta.days  # Adding 1 to include the end date

            # Check if days is zero to avoid division by zero
            if days == 0:
                # Handle the case where days is zero, maybe set revenue_calc to 0
                revenue_calc = 0
            else:
                # Proceed with the division only if days is not zero
                revenue_calc = obj.total_revenue / days
            ...
            if start_date > close_date:
                ...
                while current_date <= end_date + relativedelta(days=1) or (end_date.month == current_date.month and end_date.year == current_date.year):
                    ...
                    if d < fixed_months:
                        ...
                        if rp < 1:
                            ...
                            revenue_calc_ex = (
                                obj.total_revenue-fixed_value)/days_in_sub
                ...
                try:
                    ...
                    mth = 0
                # mth = obj.productp_service.revenue_type.months
                print("0000000000000000000000000", mth)
                end_date = start_date+relativedelta(months=mth)
                delta = end_date - start_date
                days = delta.days+1
                revenue_calc = obj.total_revenue/days
