...
def load_membership_data(file, user):
    ...
    return insert_item_to_db(tmp_data, user)

def convert_to_datetime(date_str):
    if not date_str:  # Check if the date_str is None or an empty string
        return None
    try:
        ...
        try:
            ...
            base_date = datetime(1899, 12, 30)
            return base_date + timedelta(days=date_float)
        except (ValueError, TypeError):
            return None

def insert_item_to_db(tmp_data, user):
    item_info = []
    numberss = []
    correct_csv_date = []
    num = Transaction.objects.filter(user__company=user.company).values_list('invoice_number', flat=True)
    ...
    for _, row in tmp_data.iterrows():
        ...
        start_date = convert_to_datetime(row['Subscription Start Date'])
        end_date = convert_to_datetime(row['Subscription End Date'])
        arr = row['Amount'] if pd.notnull(row['Amount']) else None
        ...
        cus_name = customer_name[0].rstrip('.')
        if row['Product/Service'] in product_name_list:
            ...
            if productp_service.revenue_type.revenue_type == "over life of subscription" and (not start_date or not end_date):
                correct_csv_date.append(row['Number'])
            else:
                if row['Number'] in numberss:
                    transaction = Transaction.objects.filter(
                        user__company=user.company, invoice_number=row['Number'],
                        customer_name__iexact=cus_name, order_close_data=row['Date']
                    ).first()

                    if transaction:
                        if not Item.objects.filter(item_description=row['Memo/Description'], quantity=abs(row['Qty']),
                        ...
                        red_flag = True
                        transaction = Transaction.objects.create(customer_name=cus_name,
                                                                 f_cst_name=customer_name[1] if len(customer_name) > 1 else cus_name,
            ...
            if row['Product/Service'] not in undefiend_service:
                undefiend_service.append(row['Product/Service'])
                ProductService.objects.create(product_name=row['Product/Service'], is_active=False, user=user)

    ProductService.objects.bulk_create([ProductService(product_name=name, is_active=False, user=user) for name in undefiend_service])
    ...
    return {"message": "please correct the date in these invoices or change product service", "correct_csv_date": correct_csv_date}
...
#             arr = row['Amount']
#             try:
#                 print(row['Subscription Start Date'], "::::::::::::::::::::",row['Number'],":::::::::::::::::::::::::",row['Date'])
...
#                         # if (len(Transaction.objects.filter(user__company = user.company, invoice_number = row['Number'], customer_name=custom_name))>0 or
#                         #     len(Transaction.objects.filter(user__company = user.company, invoice_number = row['Number'], order_close_data=row['Date']))>0):
